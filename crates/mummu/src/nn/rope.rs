...
mod tests {
    ...
    fn to_vec(t: Tensor<Cpu, 4>) -> Vec<f32> {
    ...
    fn rope_tables_position_zero_is_identity_rotation() {
        ...
        let (cos, sin) = rope_tables::<Cpu>(1, 0, 8, 1e6, &device);
    ...
    fn rope_tables_are_unit_norm_and_offset_consistent() {
        ...
        let (cos, sin) = rope_tables::<Cpu>(3, 2, 16, 1e4, &device);
        ...
        let (cos_dec, sin_dec) = rope_tables::<Cpu>(1, 4, 16, 1e4, &device);
    ...
    fn rotate_half_swaps_and_negates() {
        let device = Dev::default();
        let x = Tensor::<Cpu, 1>::from_floats([1.0, 2.0, 3.0, 4.0], &device).reshape([1, 1, 1, 4]);
    ...
    fn apply_rope_at_position_zero_is_identity() {
        let device = Dev::default();
        let x = Tensor::<Cpu, 1>::from_floats([0.5, -1.5, 2.0, 3.5], &device).reshape([1, 1, 1, 4]);
        let (cos, sin) = rope_tables::<Cpu>(1, 0, 4, 1e6, &device);
    ...
    fn rope_tables_rejects_odd_head_dim() {
        let device = Dev::default();
        let _ = rope_tables::<Cpu>(1, 0, 7, 1e6, &device);
    }
    ...
    fn rope_tables_rejects_runaway_positions() {
        let device = Dev::default();
        let _ = rope_tables::<Cpu>(1, MAX_CONTEXT_TOKENS, 8, 1e6, &device);
    }
}
