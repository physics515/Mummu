...
impl GgufFile {
    ...
    pub fn dequant_to_safetensors(
        ...
        // Resolve the whole mapping first: skipped tensors (an explicit
        // [`GgufMap::Skip`], e.g. qwen35's unused NextN block) count toward
        // neither the size bound nor the blob.
        let mut actions: Vec<Option<(String, Vec<u64>)>> =
            Vec::with_capacity(self.tensors.len());
        for (index, info) in self.tensors.iter().enumerate() {
            let action = match map(info) {
                Some(GgufMap::Rename(name)) => {
                    Some((name, info.dims.iter().rev().copied().collect::<Vec<u64>>()))
                }
        ...
        let total_f32_bytes: u64 = self
            .tensors
            .iter()
            .zip(&actions)
            .filter(|(_, a)| a.is_some())
            .map(|(t, _)| t.element_count() * 4)
            .sum();
        ...
        let mut first = true;
        for (index, (info, action)) in self.tensors.iter().zip(&actions).enumerate() {
            let Some((name, shape)) = action else {
                continue;
            };
