...
fn real_weight_tensor_through_the_twin_stays_inside_its_bound_and_is_faster() {
    ...
    let gguf = mummu::gguf::GgufFile::open(&path).expect("gguf opens");
    ...
    let info = gguf.tensor(name).expect("gate tensor exists");
    // ggml dims are fastest-varying first: [in, out] reversed.
    let (out_w, in_w) = (
        usize::try_from(info.dims[1]).expect("out dim fits usize"),
        usize::try_from(info.dims[0]).expect("in dim fits usize"),
    );
    let vals = gguf.read_tensor_f32(name).expect("dequantizes"); // row-major [out, in]
    drop(gguf);
    ...
    let weight = Tensor::<2>::from_data(TensorData::new(linear, [in_w, out_w]), &device);
    let wq = mummu::quant::quantize_weight(mummu::quant::QuantPolicy::Q4, weight);
    ...
    let x_host: Vec<f32> = (0..in_w)
        .map(|i| {
            let t = f32_from_usize(i) * 0.7311;
            0.1f32.mul_add((t * 13.7).sin().powi(3), t.sin())
        })
        .collect();
    let act = Tensor::<2>::from_data(TensorData::new(x_host.clone(), [1, in_w]), &device);

    let time_path = |label: &str, twin: bool| -> (Vec<f32>, f64) {
        mummu::flex::registry::force_disable(!twin);
        let out = mummu::nn::try_q4s_gemv(&act, &wq).expect("packed path engages");
        let mut best = f64::INFINITY;
        for _ in 0..10 {
            let t0 = std::time::Instant::now();
            let _ = mummu::nn::try_q4s_gemv(&act, &wq).expect("packed path engages");
            best = best.min(t0.elapsed().as_secs_f64() * 1e3);
        }
        let row = out.into_data().try_to_vec::<f32>().expect("readback");
    ...
    for n in 0..out_w {
        let mut req = 0.0f32;
        for k in 0..in_w {
            req = (dev_grid[k * out_w + n] - twin_grid[k * out_w + n])
                .abs()
                .mul_add(x_host[k].abs(), req);
        }
        let budget = 1e-5f32.mul_add(scale, req + act_bound[n]);
