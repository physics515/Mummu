...
fn olmoe_per_expert_q8_agrees_with_f32_and_answers() {
    ...
    let f32_top = {
        let loaded = olmoe::load_from_gguf::<Cpu>(&path, &device).expect("f32 load");
        loaded.first_token(&prompt, 1, &device).expect("probe")[0]
    };

    let q = olmoe::load_from_gguf_quantized::<Cpu>(&path, &device, QuantPolicy::Q8)
        .expect("q8 load");
    let q_top = q.first_token(&prompt, 1, &device).expect("probe")[0];
    assert_eq!(q_top, f32_top, "per-expert Q8 first-token argmax diverges from f32");
