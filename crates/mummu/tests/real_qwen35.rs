...
fn qwen35_q8_agrees_with_f32_and_stays_correct() {
    ...
    let f32_top = {
        let loaded = qwen35::load_from_gguf::<Cpu>(&path, &device).expect("f32 load");
        loaded.first_token(&prompt, 1, &device).expect("probe")[0]
    };

    let q8 = qwen35::load_from_gguf_quantized::<Cpu>(
        &path,
        &device,
        mummu::quant::QuantPolicy::Q8,
    )
    .expect("q8 load");
    let q8_top = q8.first_token(&prompt, 1, &device).expect("probe")[0];
    assert_eq!(
        q8_top, f32_top,
        "Q8's first-token argmax diverges from f32"
    );
...
#[test]
#[ignore = "needs multi-GB local weights (MUMMU_QWEN35_GGUF)"]
fn qwen35_greedy_decodes_coherent_text_on_default_device() {
    let Some(path) = gguf_path() else {
        panic!("set MUMMU_QWEN35_GGUF to a qwen35 GGUF file");
    };
    let f = mummu::gguf::GgufFile::open(&path).expect("gguf opens");
    ...
    let (ids, label) = if use_gpu() {
        ...
        let loaded = qwen35::load_from_gguf::<Gpu>(&path, &device).expect("weights load checked");
        (
            loaded
                .generate(
                    &prompt,
        ...
        let device = burn::tensor::Device::<Cpu>::default();
        let loaded = qwen35::load_from_gguf::<Cpu>(&path, &device).expect("weights load checked");
        (
            loaded
                .generate(
                    &prompt,
    ...
    assert!(!ids.is_empty(), "greedy decode produced no tokens before EOS");
    let text = tok.decode(&ids, true).expect("ids decode");
