...
fn main() {
    ...
    println!("CPU->GPU->readback [1,5120]: {:.3} ms", t.elapsed().as_secs_f64()*1e3/f64::from(n));
    ...
    println!("GPU->CPU->readback [1,5120]: {:.3} ms", t.elapsed().as_secs_f64()*1e3/f64::from(n));
    ...
    println!("GPU matmul [1,5120]x[5120,2176] + sync: {:.3} ms", per*1e3);
    println!("  x65 layers = {:.2} s/token of GPU round trips", per*65.0);
}
