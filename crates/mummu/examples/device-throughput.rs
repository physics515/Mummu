...
/// Largest 2-D weight considered, in parameters: past this the probe's own
/// allocations, not the device, decide the number.
...
fn time_matmul(device: &Device, w: &Tensor<2>, k: usize, m: usize, rounds: usize) -> Option<f64> {
    ...
    std::panic::catch_unwind(std::panic::AssertUnwindSafe(move || {
        ...
        Some(started.elapsed().as_secs_f64() * 1000.0 / f64_from_usize(rounds))
    }))
    .ok()
    .flatten()
}

/// Weight traffic in GB/s for a matmul that took `ms`. Decode is
/// bandwidth-bound, so bytes/second is the number that predicts tok/s — not
/// FLOPs.
fn weight_gbs(weight_bytes: usize, ms: f64) -> f64 {
    f64_from_usize(weight_bytes) / (ms / 1000.0) / 1e9
}

/// Every device alone. m=1 is decode, the shape that decides tok/s. m=16
/// stands in for a small prefill, where there is enough arithmetic to hide
/// latency.
fn each_device_alone(pack: &Pack, entry: &TensorEntry, k: usize, weight_bytes: usize) {
    ...
    for m in [1usize, 16] {
        ...
        for (label, device) in &devices {
            ...
            match time_matmul(device, &w, k, m, 5) {
                ...
                Some(ms) => {
                    let gbs = weight_gbs(weight_bytes, ms);
...
/// The CPU alone, by how the weight is stored. 0.2 GB/s on a DDR5 machine is
/// far too slow to be "the CPU is slow"; it points at the *path*. Host RAM is
/// not the scarce resource here (96 GiB), so if float is faster then
/// CPU-resident tensors should simply be stored dequantized.
fn cpu_by_storage(pack: &Pack, entry: &TensorEntry, k: usize, weight_bytes: usize) {
    println!(
        "
CPU only, by storage (m=1):"
    ...
    for (label, w) in [
        ...
        (
            ...
            pack.tensor::<2>(entry, Precision::Q4, &cpu)
                .ok()
                .map(|w| w.dequantize().cast(DType::F16)),
        ),
        (
            "F16 from pack",
...
/// both at once and compare against each alone. Anything close to the slower
/// of the two means they are sharing one memory controller and the second
/// device is buying nothing.
fn cpu_and_igpu_together(pack: &Pack, entry: &TensorEntry, k: usize) {
...
fn main() {
    ...
    println!(
        ...
        f64_from_usize(weight_bytes) / f64::from(1 << 20)
    );

    let hook = std::panic::take_hook();
    std::panic::set_hook(Box::new(|_| {}));

    each_device_alone(&pack, entry, k, weight_bytes);
    cpu_by_storage(&pack, entry, k, weight_bytes);
    cpu_and_igpu_together(&pack, entry, k);
