...
fn main() {
    ...
    macro_rules! time {
        ($label:expr, $body:expr) => {{
            let _ = $body; // warm
            let t = Instant::now();
            for _ in 0..n { let _ = $body; }
            println!("{:<42} {:>8.3} ms", $label, t.elapsed().as_secs_f64()*1e3/f64::from(n));
        }};
    }

    time!("s.mul(g.exp())            [decay]", s0.clone().mul(g_t.clone().exp()));
    time!("s.mul(k^T).sum_dim(2)     [v_hat]", s0.clone().mul(k_t.clone().swap_dims(2,3)).sum_dim(2));
    time!("k^T.matmul(d)             [outer]", k_t.clone().swap_dims(2,3).matmul(v_t.clone()));
    time!("s.add(outer)              [update]", s0.clone().add(s0.clone()));
    time!("s.mul(q^T).sum_dim(2)     [output]", s0.clone().mul(q_t.clone().swap_dims(2,3)).sum_dim(2));
    ...
    println!("\nFULL DeltaNet step (1 layer, 1 token): {:.3} ms", per_step*1e3);
    println!("x48 DeltaNet layers                  : {:.2} s/token", per_step*48.0);
}
