...
/// Load `entry` at `precision` onto the GPU (dequantized first when `deq`),
/// warm the matmul once and time a second call. Returns the warm result and
/// the timed call's milliseconds, or `None` when any step panicked.
fn gpu_matmul(
    pack: &Pack,
    entry: &TensorEntry,
    precision: Precision,
...
/// One round's verdict: the last panic's reason, a shape mismatch, or the
/// worst relative error against `want` with the warm timing.
fn verdict(
    got: Option<(Vec<f32>, f32)>,
    want: &[f32],
    scale: f32,
...
fn main() {
    ...
    let hook = std::panic::take_hook();
    {
        ...
        std::panic::set_hook(Box::new(move |info| {
            *last
                .lock()
                .unwrap_or_else(std::sync::PoisonError::into_inner) = info.to_string();
        }));
    }
        ...
        for precision in [Precision::Q4, Precision::Q8] {
            for (path, deq) in [("native", false), ("dequantize", true)] {
                for round in 1..=ROUNDS {
                    let got = gpu_matmul(&pack, entry, precision, deq, &xs, &gpu);
                    let verdict = verdict(got, &want, scale, &last);
